...
weighted_net_score <- function(data, var, weight_col) {
  result <- data %>%
    ...
    mutate(
      ...
      negative_weighted = if_else(.data[[var]] %in% c("A lot worse", "A little worse", "Wrong time"), .data[[weight_col]], 0)
    ) %>%
    summarise(
...
save_money_net <- df %>%
  ...
  summarise(
    ...
    pct_positive = sum(positive_weighted) / total_weight * 100,
    pct_negative = sum(negative_weighted) / total_weight * 100,
    net_score = round(pct_positive - pct_negative)
  ) %>%
  pull(net_score)
